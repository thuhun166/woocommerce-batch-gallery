add_action('init', function () {
    ...
    if (!is_admin()) return;
    if (get_option('rfs_gallery_done') === 'yes') return;

    $existing = get_page_by_title('RFS - Delivery Information', OBJECT, 'attachment');
    if (!$existing) return;
    $image_id = $existing->ID;
    $offset = (int) get_option('rfs_gallery_offset', 0);
    $per_page = 200;
    ...
    if (empty($products)) {
        update_option('rfs_gallery_done', 'yes');
        return;
    }
    ...
    update_option('rfs_gallery_offset', $offset + $per_page);
